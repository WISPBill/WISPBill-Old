...
WHERE  `firewall_idfirewall` =  '$firewallid'
ORDER BY  `rule_number` DESC ")) {
						 /* fetch associative array */
						 while ($row4 = $result4->fetch_assoc()) {
						 $oldrule= $row4["rule_number"];
						 }
}
$rejectrule = $oldrule +1;
   $mac = strtolower($mac);                           
                         $ssh->exec("/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper begin\n
/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper set firewall modify $name rule $rejectrule action modify\n 
/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper set firewall modify $name rule $rejectrule modify table 35\n
/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper set firewall modify $name rule $rejectrule protocol all\n
/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper set firewall modify $name rule $rejectrule source mac-address '$mac'\n
/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper commit\n
/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper save\n");
...
WHERE  `firewall_idfirewall` =  '$firewallid' and `customer_info_idcustomer_info` = '$cusid'")) {
                         ...
                         $ssh->exec("/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper begin\n
/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper delete firewall name $name rule $oldrule\n 
/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper commit\n
/opt/vyatta/sbin/vyatta-cfg-cmd-wrapper save\n");
						 
 if ($result = $mysqli->query("DELETE FROM `Firewall_Rules` WHERE `idACL` = '$ruleid'")) {
