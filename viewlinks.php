...
if ($result = $mysqli->query("SELECT * FROM  `links` ")) {
     ...
     echo "<h3 class=\"box-title\">View Links</h3>";			
 ?>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
				  ...
				  <th>Slave Port</th>
                  <th>Capacity</th>
                  <th>Peak Utilization in the last week</th>
                </tr>
                </thead>
                <tbody>
				 <?php
     ...
     $slavesite= $row["slave_site"];
     $slaveportid= $row["slave_port"];
     $masterportid= $row["master_port"];
     
     if ($result2 = $mysqli->query("SELECT * FROM `device_ports` WHERE `iddevice_ports` = '$slaveportid'")) {
      while ($row2 = $result2->fetch_assoc()) {
     $portname= $row2["name"];
     $ipadd= $row2["ip_address"];
     
     $slaveport = "$portname $ipadd";
      }
     }
     
     if ($result2 = $mysqli->query("SELECT * FROM `device_ports` WHERE `iddevice_ports` = '$masterportid'")) {
      while ($row2 = $result2->fetch_assoc()) {
     $portname= $row2["name"];
     $ipadd= $row2["ip_address"];
     
     $masterport = "$portname $ipadd";
      }
     }
     
     if ($result4 = $mysqli->query("SELECT * FROM `location` WHERE `idlocation` = '$slavesite'")) {
      while ($row4 = $result4->fetch_assoc()) {
     $slavesitename= $row4["name"];
      }
     }
     if ($result4 = $mysqli->query("SELECT * FROM `location` WHERE `idlocation` = '$mastersite'")) {
      while ($row4 = $result4->fetch_assoc()) {
     $mastersitename= $row4["name"];
      }
     }
     $weekago = time() - 604800;
     if ($result4 = $mysqli->query("SELECT MAX(  txrate ) FROM port_data
WHERE  `time` >  '$weekago' and `device_ports_iddevice_ports` = '$masterportid'")) {
      while ($row4 = $result4->fetch_assoc()) {
     $maxtx= $row4["MAX(  txrate )"];
      }
     }
     
     if ($result4 = $mysqli->query("SELECT MAX(  rxrate ) FROM port_data
WHERE  `time` >  '$weekago' and `device_ports_iddevice_ports` = '$masterportid'")) {
      while ($row4 = $result4->fetch_assoc()) {
     $maxrx= $row4["MAX(  rxrate )"];
      }
     }
       
       $largerrate = max("$maxtx","$maxrx");
       
       $mbpsrate = $largerrate / 1000000;
       
       $uti = $mbpsrate / $capacity;
       
       $uti = number_format( $uti * 100, 2 ) . '%';
     echo" <tr>
    <td>$mastersitename</td> 
    <td>$slavesitename</td>
    <td>$masterport</td> 
    <td>$slaveport</td>
    <td>$capacity</td>
