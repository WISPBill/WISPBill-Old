...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
      Convert Discovered Device to Inventory
      </h1>
      <ol class="breadcrumb">
        <li><a href="dashbored.php"><i class="fa fa-dashboard"></i> Dashbored</a></li>
        <li class="active">Convert Discovered</li>
      </ol>
    </section>

    <!-- Main content -->
	 <form role="form" action="createdevice2.php?source=convert"method="post">
	<div class="row">
        <div class="col-xs-12">
...
$error = $_SESSION['exitcodev2'];
                ...
                if(isset($_GET["error"])){
				 echo '<h3 class="box-title" style="color: red;">There Were Errors Please Reselect  </h3>';
				}else{
...
?> 
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
				 <?php
                if ($result = $mysqli->query("SELECT * FROM  `ip_address` WHERE  `devices_iddevices` IS NULL ")) {
      /* fetch associative array */
         $ids = array();
    while ($row = $result->fetch_assoc()) {
     $id= $row["idip_address"];
  ...
  $response = curl_exec($ch);
    ...
    if($status == '0'){
     ...
     echo" <tr>
    <td><input id='$id' type='radio' name='id' value=$id unchecked></td>
    <td>$address</td> 
    <td>$mac</td>
    <td>$vendor</td>
    <td>$status</td> 
...
?>
  ...
  <script type='text/javascript'>
    ...
    foreach($ids as $id){
        echo "
  $('#$id').change(function() {
  $('#third-choice').load('$url/getadddevice.php?choice=' + $('#$id').val());});
   ";
    }
  ?>
 </script>
