...
<?php
	         $mysqli = new mysqli("$ip", "$username", "$password", "$db");
if ($result = $mysqli->query("SELECT * FROM `dhcp_servers`")) {
      /* fetch associative array */

     echo "<h3 class=\"box-title\">View DHCP Servers</h3>";			
 ?>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
     ...
     $rangestop= $row["Range_Stop"];
     $subnet= $row["subnet"];
     $dns= $row["DNS"];
     $portid= $row["device_ports_iddevice_ports"];
     
     
     if ($result2 = $mysqli->query("SELECT * FROM `device_ports` WHERE `iddevice_ports` = '$portid'")) {
     ...
     if ($result3 = $mysqli->query("SELECT * FROM `devices` WHERE `iddevices` = '$did'")) {
      while ($row3 = $result3->fetch_assoc()) {
     $site= $row3["location_idlocation"];
      }
     }
     if ($result4 = $mysqli->query("SELECT * FROM `location` WHERE `idlocation` = '$site'")) {
