...
<head>
  ...
  <link 
        rel="stylesheet" 
        href="dist/leaflet.draw.css"
    />
  <script src="leaflet/leaflet.js"></script>
    <script
        src="dist/leaflet.draw.js">
    </script>
  <style>
...
$error = $_SESSION['errorcode'];
                if($error =='cov'){
				 echo '<h3 class="box-title" style="color: red;">You need to Draw a Poylgon</h3>';
				}else{
				 echo '<h3 class="box-title">Draw Coverage</h3>';
				}
?> 
      <ol class="breadcrumb">
        <li><a href="dashbored.php"><i class="fa fa-dashboard"></i> Dashbored</a></li>
        <li class="active">Draw Coverage Map</li>
      </ol>
    </section>

    <!-- Main content -->
    <section class="content">
      <form role="form" action="drawcov2.php"method="post">
  ...
  var drawnItems = new L.FeatureGroup();
        ...
        map.on('draw:created', function (e) {
            ...
            drawnItems.addLayer(layer);
             ...
             var shape = layer.toGeoJSON(drawnItems)
            var db = JSON.stringify(shape);
            document.getElementById("data").value = db;
        });
        
        <?php
$mysqli = new mysqli("$ip", "$username", "$password", "$db");

if ($result = $mysqli->query("SELECT * FROM `location` WHERE `idlocation` = $id")) {
      /* fetch associative array */
