    ...
    <section class="content">
         ...
         <!-- general form elements disabled -->
          <div class="box box-warning">
            ...
            <!-- /.box-header -->
            <div class="box-body">
              <form role="form" action="addlink2.php"method="post">
                <!-- text input -->
                 ...
                 <div class="form-group">
					...
					if($error == 'capacity'){
						...
						echo '<label>Capacity in Mbps</label>';
					}
					?>
                  <input type="number" min="0" class="form-control" name="capacity" placeholder="Enter Capacity" required>
                </div>
...
WHERE  `ifconfig` =  'yes'")) {
    /* fetch associative array */
                ...
                if($error == 'site'){
						...
						echo '<label>Select the Master Site</label>';
					}
 ...
 echo ' </select>
              ...
              if($error == 'port'){
						...
						echo '<label>Select the Master Port</label>';
					}
                    echo '
                    <div class="form-group">
                <select class="form-control" name="port" id="third-choice" required>
...
WHERE  `ifconfig` =  'yes'")) {
    /* fetch associative array */
                ...
                if($error == 'slavesite'){
						...
						echo '<label>Select the Slave Site</label>';
					}
                echo '<select class="form-control" name="slavesite" id="slavefirst-choice" required>
				  <option value="" selected disabled>Please Select Site</option>';
    while ($row = $result->fetch_assoc()) {
        $id = $row["idlocation"];
        $name = $row["name"];
 ...
 echo ' </select>
              ...
              if($error == 'slaveport'){
						...
						echo '<label>Select the Slave Port</label>';
					}
                    echo '
                    <div class="form-group">
                <select class="form-control" name="slaveport" id="slavethird-choice" required>
				  <option value="" selected disabled>Please Select Site First</option>
                </select>
                </div>
  ...
  $("#third-choice").load("<?php echo"$url"; ?>/portgetall.php?choice=" + $("#first-choice").val());});
    ...
    <script type="text/javascript">
  $("#slavefirst-choice").change(function() {
  $("#slavethird-choice").load("<?php echo"$url"; ?>/portgetall.php?choice=" + $("#slavefirst-choice").val());});
  </script>
    </section>
    <!-- /.content -->
  </div>
