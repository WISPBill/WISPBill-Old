            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
				 <?php
         $mysqli = new mysqli("$ip", "$username", "$password", "$db");
 if ($result = $mysqli->query("SELECT * FROM `contact_notes`")) {
    /* fetch associative array */
    foreach ($result as $row){
		 $note = $row["note"];
        $date = $row["date"];
		 $cid = $row["contacts_idcontacts"];
		 $taid = $row["admin_users_idadmin"];
			if ($result2 = $mysqli->query("SELECT * FROM `contacts` WHERE `idcontacts` = $cid")) {
				/* fetch associative array */
