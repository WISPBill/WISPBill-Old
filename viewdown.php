  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Down Devices</li>
      </ol>
    </section>
...
WHERE  `iddevices` IN 
(SELECT `devices_iddevices` FROM `ip_address` WHERE `status` = '0' and `devices_iddevices` is not NULL
)
 AND  `field_status` != 'inventory'")) {
      /* fetch associative array */

     echo "<h3 class=\"box-title\">Down Devices</h3>";			
 ?>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
     ...
     if ($result2 = $mysqli->query("SELECT * FROM `location` WHERE `idlocation` = '$site'")) {
      while ($row2 = $result2->fetch_assoc()) {
     $sitename= $row2["name"];
      }
     }
     
     if ($result3 = $mysqli->query("SELECT * FROM `ip_address` WHERE `devices_iddevices` = '$did'")) {
